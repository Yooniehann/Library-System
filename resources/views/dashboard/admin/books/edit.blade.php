            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="mb-4">
                    ...
                    <select name="publisher_id" id="publisher_id" 
                        ...
                        @foreach($publishers as $publisher)
                            <option value="{{ $publisher->publisher_id }}" {{ (old('publisher_id', $book->publisher_id) == $publisher->publisher_id) ? 'selected' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="publication_year" id="publication_year" 
                           class="bg-slate-700 text-white border border-gray-600 rounded w-full py-2 px-3 focus:outline-none focus:ring-2 focus:ring-yellow-400"
                           min="1000" max="{{ date('Y') + 1 }}" required value="{{ old('publication_year', $book->publication_year) }}">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="language" id="language" 
                           class="bg-slate-700 text-white border border-gray-600 rounded w-full py-2 px-3 focus:outline-none focus:ring-2 focus:ring-yellow-400"
                           required value="{{ old('language', $book->language) }}">
                    @error('language')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" step="0.01" name="pricing" id="pricing" min="0"
                           class="bg-slate-700 text-white border border-gray-600 rounded w-full py-2 px-3 focus:outline-none focus:ring-2 focus:ring-yellow-400"
                           required value="{{ old('pricing', $book->pricing) }}">
                    @error('pricing')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4 md:col-span-2">
                    ...
                    @if($book->cover_image)
                        <div class="mb-2">
                            <img src="{{ asset('storage/' . $book->cover_image) }}" alt="Current cover" class="h-32 w-24 object-cover rounded">
                        </div>
                    @endif
                ...
                <div class="mb-4 md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="5"
                              class="bg-slate-700 text-white border border-gray-600 rounded w-full py-2 px-3 focus:outline-none focus:ring-2 focus:ring-yellow-400">{{ old('description', $book->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
