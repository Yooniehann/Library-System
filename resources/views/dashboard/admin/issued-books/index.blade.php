                ...
                <form method="GET" action="{{ route('admin.issued-books.index') }}">
                    <div class="relative flex">
                        <input type="text"
                               ...
                               placeholder="Search by ID, user, book, author..."
                               name="search"
                               value="{{ $searchTerm }}">
                        <button type="submit"
            ...
            <table class="min-w-full divide-y divide-gray-700">
                <thead class="bg-slate-700">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Renewals</th>
                ...
                <tbody class="bg-slate-800 divide-y divide-gray-700">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-white">{{ Str::limit($borrow->inventory->book->title, 30) }}</td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if($borrow->status == 'active')
                                <span class="px-2 py-1 text-xs font-semibold bg-green-500/20 text-green-400 rounded-full">Active</span>
                            @elseif($borrow->status == 'overdue')
                                <span class="px-2 py-1 text-xs font-semibold bg-red-500/20 text-red-400 rounded-full">Overdue</span>
                            @else
                                <span class="px-2 py-1 text-xs font-semibold bg-gray-500/20 text-gray-400 rounded-full">{{ ucfirst($borrow->status) }}</span>
                            @endif
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{{ $borrow->renewal_count }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex space-x-2">
                                ...
                                <form action="{{ route('admin.issued-books.renew', $borrow->borrow_id) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="text-green-400 hover:text-green-500" title="Renew Book">
    ...
    setInterval(function() {
        fetch('{{ route("admin.issued-books.stats") }}')
            ...
            .then(data => {
                ...
                document.querySelector('[data-stat="total_fines"]').textContent = '$' + data.total_fines.toFixed(2);
            });
    }, 60000);
</script>
@endsection
