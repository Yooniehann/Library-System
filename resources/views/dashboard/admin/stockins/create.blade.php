...
<div class="container mx-auto px-4 py-6">
    <h1 class="text-2xl font-bold text-white mb-6">Create New Stock In</h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="mb-4">
                    ...
                    <select name="staff_id" id="staff_id" 
                        ...
                        @foreach($staff as $staffMember)
                            <option value="{{ $staffMember->staff_id }}" {{ old('staff_id', isset($stockin) ? $stockin->staff_id : '') == $staffMember->staff_id ? 'selected' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    <input type="date" name="stockin_date" id="stockin_date" 
                           class="bg-slate-700 text-white border border-gray-600 rounded w-full py-2 px-3 focus:outline-none focus:ring-2 focus:ring-yellow-400"
                           value="{{ old('stockin_date', date('Y-m-d')) }}" required>
                    @error('stockin_date')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
