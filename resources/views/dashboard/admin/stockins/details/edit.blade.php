...
<div class="container mx-auto px-4 py-6">
    <h1 class="text-2xl font-bold text-white mb-6">Edit Book in Stock In #{{ $stockin->stockin_id }}</h1>

    <div class="bg-slate-800 rounded-lg shadow p-6">
        <form action="{{ route('admin.stockins.details.update', ['stockin' => $stockin, 'detail' => $detail]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="mb-4">
                    ...
                    <select name="book_id" id="book_id" 
                        ...
                        @foreach($books as $book)
                            <option value="{{ $book->book_id }}" {{ $selectedBookId == $book->book_id ? 'selected' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="received_quantity" id="received_quantity" min="1"
                           class="bg-slate-700 text-white border border-gray-600 rounded w-full py-2 px-3 focus:outline-none focus:ring-2 focus:ring-yellow-400"
                           value="{{ old('received_quantity', $detail->received_quantity) }}" required>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="price_per_unit" id="price_per_unit" step="0.01" min="0"
                           class="bg-slate-700 text-white border border-gray-600 rounded w-full py-2 px-3 focus:outline-none focus:ring-2 focus:ring-yellow-400"
                           value="{{ old('price_per_unit', $detail->price_per_unit) }}" required>
                ...
                <div class="mb-4 md:col-span-2">
                    ...
                    <textarea name="remarks" id="remarks" rows="3"
                              class="bg-slate-700 text-white border border-gray-600 rounded w-full py-2 px-3 focus:outline-none focus:ring-2 focus:ring-yellow-400">{{ old('remarks', $detail->remarks) }}</textarea>
                    @error('remarks')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
