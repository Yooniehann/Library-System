                ...
                <div>
                    ...
                    <span class="px-2 py-1 text-xs rounded-full text-white" @class([
                        ...
                        'bg-yellow-500 text-black' =>
                            $stockin->status !== 'Received' && $stockin->status !== 'Canceled',
                    ])>
                        {{ $stockin->status }}
                    </span>
                </div>
                    ...
                    <tbody class="bg-slate-800 divide-y divide-gray-700">
                        @foreach ($stockin->details as $detail)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                                    ${{ number_format($detail->price_per_unit, 2) }}</td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex space-x-2">
                                        ...
                                        <form
                                            action="{{ route('admin.stockins.details.destroy', ['stockin' => $stockin, 'detail' => $detail]) }}"
                                            method="POST"
