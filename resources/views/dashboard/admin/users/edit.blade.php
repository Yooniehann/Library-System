        ...
        <form action="{{ route('admin.users.update', $user) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2">
                    <h2 class="text-xl font-semibold text-yellow-400 mb-4 border-b border-gray-700 pb-2">Personal Information</h2>
                </div>
                ...
                <div class="mb-4">
                    <label for="password" class="block text-gray-300 text-sm font-bold mb-2">New Password (leave blank to keep current)</label>
                ...
                <div class="mb-4">
                    <label for="password_confirmation" class="block text-gray-300 text-sm font-bold mb-2">Confirm New Password</label>
                ...
                <div class="mb-4">
                    <label for="date_of_birth" class="block text-gray-300 text-sm font-bold mb-2">Date of Birth *</label>
                ...
                <div class="mb-4 md:col-span-2">
                    ...
                    <textarea name="address" id="address" rows="3"
                              class="bg-slate-700 text-white border border-gray-600 rounded w-full py-2 px-3 focus:outline-none focus:ring-2 focus:ring-yellow-400">{{ old('address', $user->address) }}</textarea>
                    @error('address')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <!-- Account Information -->
                <div class="md:col-span-2">
                    <h2 class="text-xl font-semibold text-yellow-400 mb-4 border-b border-gray-700 pb-2">Account Information</h2>
                </div>
                ...
                <div class="mb-4 flex items-center">
                    <input type="checkbox" name="is_kid" id="is_kid" value="1" {{ old('is_kid', $user->is_kid) ? 'checked' : '' }} 
                ...
                <div class="md:col-span-2">
                    <h2 class="text-xl font-semibold text-yellow-400 mb-4 border-b border-gray-700 pb-2">Membership Information</h2>
                </div>

                <div class="mb-4">
                    <label for="membership_type_id" class="block text-gray-300 text-sm font-bold mb-2">Membership Type</label>
                    <select name="membership_type_id" id="membership_type_id"
                           class="bg-slate-700 text-white border border-gray-600 rounded w-full py-2 px-3 focus:outline-none focus:ring-2 focus:ring-yellow-400">
                        <option value="">Select Membership Type</option>
                        @foreach($membershipTypes as $type)
                            <option value="{{ $type->membership_type_id }}" {{ old('membership_type_id', $user->membership_type_id) == $type->membership_type_id ? 'selected' : '' }}>
                ...
                <div class="mb-4">
                    <label for="membership_period" class="block text-gray-300 text-sm font-bold mb-2">Billing Period</label>
                    <select name="membership_period" id="membership_period"
                           class="bg-slate-700 text-white border border-gray-600 rounded w-full py-2 px-3 focus:outline-none focus:ring-2 focus:ring-yellow-400">
                        <option value="monthly" {{ old('membership_period', 'monthly') == 'monthly' ? 'selected' : '' }}>Monthly</option>
                        <option value="yearly" {{ old('membership_period') == 'yearly' ? 'selected' : '' }}>Yearly</option>
                    </select>
                ...
                <div class="mb-4">
                    <label for="membership_duration" class="block text-gray-300 text-sm font-bold mb-2">Duration (Months/Years)</label>
                    <input type="number" name="membership_duration" id="membership_duration" min="1" max="60"
                           class="bg-slate-700 text-white border border-gray-600 rounded w-full py-2 px-3 focus:outline-none focus:ring-2 focus:ring-yellow-400"
                ...
                <div class="mb-4">
                    <label for="membership_start_date" class="block text-gray-300 text-sm font-bold mb-2">Start Date</label>
                    <input type="date" name="membership_start_date" id="membership_start_date" 
                           class="bg-slate-700 text-white border border-gray-600 rounded w-full py-2 px-3 focus:outline-none focus:ring-2 focus:ring-yellow-400"
                           value="{{ old('membership_start_date', $user->membership_start_date ? $user->membership_start_date->format('Y-m-d') : date('Y-m-d')) }}">
                ...
                <div class="mb-4">
                    <label for="membership_end_date" class="block text-gray-300 text-sm font-bold mb-2">End Date (Auto-calculated)</label>
                    <input type="date" name="membership_end_date" id="membership_end_date" readonly
                           ...
                           value="{{ old('membership_end_date', $user->membership_end_date ? $user->membership_end_date->format('Y-m-d') : '') }}">
    ...
    if (membershipTypeSelect.value && durationInput.value) {
        ...
        if (period === 'yearly') {
            ...
            if (annualFeeText.includes('yearly') || annualFeeText.includes('annual')) {
                const annualFee = parseFloat(selectedOption.text.split('$')[2] || selectedOption.text.split('$')[1].split('or')[1]);
