    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <div class="flex flex-col flex-1 overflow-hidden">
            ...
            <div class="md:hidden flex items-center justify-between px-4 py-3 bg-black border-b border-slate-700">
                ...
                <span class="text-primary-orange text-lg font-bold">My Borrowed Books</span>
                <div class="w-6"></div>
            </div>
                                    ...
                                    <tr class="hover:bg-slate-700">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center">
                                                <div class="flex-shrink-0 h-12 w-9 bg-slate-600 flex items-center justify-center rounded">
                                                    <i class="fas fa-book text-gray-400"></i>
                                                </div>
                                                <div class="ml-4">
                                                    <div class="text-sm font-medium text-white">{{ $borrow->inventory->book->title }}</div>
                                                    <div class="text-sm text-gray-400">{{ $borrow->inventory->book->author->name ?? 'Unknown Author' }}</div>
                                                    <div class="text-xs text-gray-500">ISBN: {{ $borrow->inventory->book->isbn }}</div>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            @if($borrow->status == 'active')
                                                ...
                                                @else
                                                    <span class="px-2 py-1 text-xs font-semibold rounded-full bg-green-900 text-green-300">Active</span>
                                                @endif
                                            @else
                                                <span class="px-2 py-1 text-xs font-semibold rounded-full bg-gray-600 text-gray-300">{{ ucfirst($borrow->status) }}</span>
                                            @endif
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            @if($borrow->status == 'active')
                                            <form action="{{ route('borrow.renew', $borrow->borrow_id) }}" method="POST" class="inline">
                                                @csrf
                                                <button type="submit" class="text-primary-orange hover:text-dark-orange mr-3">
                                                    <i class="fas fa-sync-alt mr-1"></i> Renew
                                                </button>
                                            </form>
                                            @endif
                                            <form action="{{ route('book.return', $borrow->borrow_id) }}" method="POST" class="inline">
                                                @csrf
            ...
            <div class="relative flex-1 flex flex-col w-64 bg-black">
                ...
                <div class="flex-1 h-0 pt-5 pb-4 overflow-y-auto">
                    ...
                    <nav class="mt-5 px-2 space-y-1">
                        ...
                        <a href="{{ route('borrowed.index') }}" class="flex items-center px-2 py-2 text-base font-medium text-white bg-dark-orange rounded-lg">
