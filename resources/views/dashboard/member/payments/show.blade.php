            ...
            <main class="flex-1 overflow-y-auto p-4 md:p-6">
                <div class="flex justify-between items-center mb-6">
                    <div>
                        ...
                        <p class="text-gray-400">Payment #{{ $payment->payment_id }}</p>
                    </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                    ...
                    <div class="bg-slate-800 rounded-lg shadow p-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="flex justify-between">
                                ...
                                <span class="px-2 py-1 text-xs font-semibold rounded-full 
                                    {{ $payment->status === 'completed' ? 'bg-green-900 text-green-300' : 
                                       ...
                                       ($payment->status === 'failed' ? 'bg-red-900 text-red-300' : 'bg-gray-600 text-gray-300')) }}">
                            ...
                            <div class="flex justify-between">
                                ...
                                <span class="text-white">{{ ucfirst($payment->payment_method) }}</span>
                            </div>
                            
                            <div class="flex justify-between">
                                <span class="text-gray-400">Payment Date:</span>
                                <span class="text-white">{{ $payment->payment_date ? $payment->payment_date->format('M d, Y') : 'N/A' }}</span>
                            </div>
                    ...
                    <div class="bg-slate-800 rounded-lg shadow p-6">
                        ...
                        <div class="flex items-start mb-4">
                            <div class="h-16 w-12 bg-slate-700 rounded overflow-hidden shadow-md flex items-center justify-center mr-4">
                                @if($payment->fine->borrow->inventory->book->cover_image)
                                    <img src="{{ asset('storage/' . $payment->fine->borrow->inventory->book->cover_image) }}" alt="Book cover" class="h-full w-full object-cover">
                                @else
                        ...
                        <div class="space-y-2">
                            <div class="flex justify-between">
                                ...
                                <span class="text-white">#{{ $payment->fine->fine_id }}</span>
                            </div>
                            
                            <div class="flex justify-between">
                                <span class="text-gray-400">Fine Type:</span>
                                <span class="px-2 py-1 text-xs font-semibold rounded-full 
                                    {{ $payment->fine->fine_type === 'overdue' ? 'bg-yellow-900 text-yellow-300' : 
                                       ($payment->fine->fine_type === 'lost' ? 'bg-red-900 text-red-300' : 'bg-gray-600 text-gray-300') }}">
                                    {{ ucfirst($payment->fine->fine_type) }}
                                </span>
                            </div>
                            
                            <div class="flex justify-between">
                                <span class="text-gray-400">Total Fine Amount:</span>
                                <span class="text-red-400">${{ number_format($payment->fine->total_amount, 2) }}</span>
                            </div>
                            
                            <div class="flex justify-between">
                                <span class="text-gray-400">Fine Status:</span>
                                <span class="px-2 py-1 text-xs font-semibold rounded-full 
                                    {{ $payment->fine->status === 'paid' ? 'bg-green-900 text-green-300' : 
                                       ($payment->fine->status === 'waived' ? 'bg-blue-900 text-blue-300' : 'bg-red-900 text-red-300') }}">
                ...
                <div class="bg-slate-800 rounded-lg shadow p-6 mb-6">
                    <h2 class="text-lg font-semibold text-white mb-4 border-b border-slate-700 pb-2">Payment Notes</h2>
                ...
                <div class="bg-slate-800 rounded-lg shadow p-6">
                    ...
                    <div class="flex flex-col sm:flex-row gap-4">
                        ...
                        <a href="{{ route('member.fines.show', $payment->fine->fine_id) }}" class="bg-slate-700 text-white px-6 py-3 rounded-lg hover:bg-slate-600 transition-colors text-center">
                            <i class="fas fa-eye mr-2"></i> View Fine Details
                        </a>
                        @endif
                    </div>
                </div>
